...
fn add_coplanar(pts: &Vec<R3>, hull: &mut Vec<Tri>, id: isize) {
    ...
    for k in 0..numh {
        ...
        if hull[k].c == hull[hull[k].ab as usize].c { // ->  ab is an external edge.
            // test this edge for visibility from new point pts[id].
            let p1 = hull[k].a;
            let p2 = hull[k].b;
            let p3 = hull[k].c;

            let (zot, e) = vec3d::cross_test(&from_ref_r3_to_vec3d(pts), p1 as usize, p2 as usize, p3 as usize, id as usize);

            if zot < 0 { // visible edge facet, create 2 new hull plates.
                let xx = hull[k].e.dot(e);
                let mut up = TriBuilder::new(id, p1, p2).set_e(e).set_id(hull.len() as isize).set_ab(-1).set_ac(-1).set_keep(2);
                let mut down = TriBuilder::new(id, p1, p2).set_e(-e).set_id(hull.len() as isize + 1).set_ab(-1).set_ac(-1).set_keep(2);
        ...
        if hull[k].a == hull[hull[k].bc as usize].a { // bc is an external edge.
            // test this edge for visibility from new point pts[id].
            let p1 = hull[k].b;
            let p2 = hull[k].c;
            let p3 = hull[k].a;
            ...
            if zot < 0 { // visible edge facet, create 2 new hull plates.
                let mut up = TriBuilder::new(id, p1, p2).set_id(hull.len() as isize).set_e(e).set_keep(2).set_ab(-1).set_ac(-1);
                let mut down = TriBuilder::new(id, p1, p2).set_id(hull.len() as isize + 1).set_e(-e).set_keep(2).set_ab(-1).set_ac(-1);
        ...
        if hull[k].b == hull[hull[k].ac as usize].b { // ac is an external edge.
            // test this edge for visibility from new point pts[id].
            let p1 = hull[k].a;
            let p2 = hull[k].c;
            let p3 = hull[k].b;
