...
int read_R3(std::vector<R3> &pts, char *fname)
{
    ...
    if (myfile.is_open())
    {
        ...
        { // assume all number pairs on a line are points
            if (line.length() <= 512)
            {
                copy(line.begin(), line.end(), s0);
                s0[line.length()] = 0;
...
void write_Tris_stl(std::vector<Tri> &ts, std::vector<R3> &pts, char *fname)
{
    ...
    for (int r = 0; r < nr; r++)
    {
        ...
        point n;
        n.x = p12.y * p13.z - p13.y * p12.z;
        n.y = -p12.x * p13.z + p13.x * p12.z;
        n.z = p12.x * p13.y - p13.x * p12.y;
        ...
        out << std::scientific;
        out << "facet normal " << n.x << " " << n.y << " " << n.z << std::endl;
...
int NewtonApple_Delaunay(std::vector<R3> &pts, std::vector<Tri> &hulk)
{
    ...
    for (int i = 0; i < pts.size(); ++i) {
        std::cout << pts[i].r << ", " << pts[i].c << ", " << pts[i].z << ", " << pts[i].id << std::endl;
    }
...
{
    std:: cout << "ENTERING COPLANAR!" << std::endl;
...
int init_hull3D_compact(std::vector<R3> &pts, std::vector<Tri> &hull)
{
    ...
    for (int p = 3; p < nump; p++)
    { // add points until a non coplanar set of points is achieved.
        std::cout << "NUM POINTS = " << p << "=============================== " << std::endl;
        ...
        for (int i = 0; i < hull.size(); ++i) {
            if (hull[i].ab < 0 || hull[i].ac < 0 || hull[i].bc < 0) {
